...
export default class News extends Component {
                        ...
                        news.map(({ id, title, content }) => {
                            return (
                                <li key={id}>
                                    ...
                                    to={`Detail?id=${id}&title=${title}&content=${content}`}
                                    >
                                        {title}
                                    </Link>
                                    <button onClick={this.push(id,title,content)}>push</button>
                                    <button onClick={this.replace(id,title,content)}>replace</button>
                                </li>
                            )
                        })
                    }
    ...
    push = (id,title,content) => {
        // return () => {
        //     this.props.history.push(`/Home/News/Detail?id=${id}&title=${title}&content=${content}`,);
        // }
    };
    replace = (id,title,content) => {
        // return () => {
        //     this.props.history.replace(`/Home/News/Detail?id=${id}&title=${title}&content=${content}`,);
        // }
    };
}
